...
/*
    ...
    Approach :
    ----------
    Only difference is pay fee when you sell.

    solve(i, canBuy):
      if i == n: return 0

      if canBuy == 1:
        buy  = -prices[i] + solve(i+1, 0)              // buy today
        skip = solve(i+1, 1)                            // do nothing
        return max(buy, skip)

      else:
        sell = prices[i] - fee + solve(i+1, 1)          // DIFF: sell today, pay fee
        skip = solve(i+1, 0)                            // do nothing
