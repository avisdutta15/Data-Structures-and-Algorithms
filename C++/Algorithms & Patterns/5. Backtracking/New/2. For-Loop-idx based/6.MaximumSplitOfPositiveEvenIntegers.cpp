...
class Solution2 {
private:
    bool solve(long long start_num, vector<long long>& combination, long long target) {
        if (target == 0) {
            return true;
        }            

        //overshoot
        if (target < 0 || start_num>target)
            return false;

        for (long long i = start_num; i <= target; i = i + 2) {
            combination.push_back(i);
            bool recAns = solve(i + 2, combination, target - i);
            if (recAns == true)
                return true;
            combination.pop_back();
        }
...
public:
    vector<long long> maximumEvenSplit(long long finalSum) {
        ...
        vector<long long> combination;
        long long start_num = 2;
        solve(start_num, combination, finalSum);
        return combination;
    }
};
...
public:
    vector<long long> maximumEvenSplit(long long finalSum) {
        ...
        while (current_even <= finalSum) {
            ...
            finalSum = finalSum - current_even;
